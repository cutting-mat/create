<template>
  <div v-loading="loading" class="upload_image">
    <div v-for="(item, index) in list" :key="'img' + index" class="previewItem">
      <img :src="item.url" @click="handlePictureCardPreview(item)" />
      <i
        class="el-icon-delete _del"
        title="删除"
    ...
    <div class="data-list-hd">支持{{ actualAccept | typeDescription }}</div>
...
export default {
  ...
  methods: {
    ...
    handleBeforeUpload() {
      // 多选状态检测上传数限制
      if (this.multiple && this.limit && !(this.limit > this.list.length)) {
...
.upload_image >>> .el-upload-image,
.upload_image >>> .data-list-hd {
  margin: 0 8px 8px 0;
}
